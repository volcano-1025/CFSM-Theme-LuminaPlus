...
- **首页延迟柱状图不自动查历史**：`/api/history/all` 会扫该节点整段时间窗口的历史行，
  ...
  2. **后端窗口（兜底）**：`/api/servers` 的 `servers[].ping` / `servers[].loss`，20 个槽位、
     ...
     主题会**丢掉连续 4 格以上「四条线路的延迟和丢包同时逐字节相同」的段落**，那些格子留空。
...
- **主题设置存在本地，登录站长可发布到站点**：设置页右上角的「保存到本机」把配置存进当前设备的
  浏览器 localStorage，只影响这台设备。

  **登录站长**：还多一个「**保存到站点**」——把当前配置直接写到后端，所有设备与访客都会用这套。
  流程是先「保存到本机」在这台设备预览，确认后「保存到站点」发布；发布成功后本机自动跟随。
  它只更新站点的主题配置，不碰站点标题、背景图、CSP 等其它外观设置。「复制配置 JSON」对登录站长
  隐藏（有了「保存到站点」用不到了）。

  **未登录 / 纯静态或跨域部署**：没有「保存到站点」，改用「复制配置 JSON」——把导出的完整快照
  （含配色等设置页之外的项）粘到后台「外观设置 → 主题自定义配置」保存，效果一样（键名与主题设置一致）。

  **「改用站点配置」**：丢掉本机存过的设置（含配色），改用站点当前的配置。某台设备存过本机设置后，
  站点改的配置在那台设备上压不过来（本地优先），在那台设备点一次「改用站点配置」即可跟随。
