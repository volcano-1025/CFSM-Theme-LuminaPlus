...
- **首页延迟柱状图不查历史**：`/api/history/all` 会扫该节点整段时间窗口的历史行，
  ...
  **柱子以后端窗口为准**：一小时 30 格、一格对应窗口的一个槽位，丢包率数字与柱子同源。
  浏览器自己攒的实时样本只在窗口缺了一段（或某一格后端没测出值）时用来补，正常情况下
  一个都不掺 —— 混着算会让丢包的加权平均随页面开着的时长慢慢漂移。柱子因此跟着窗口
  每 2 分钟动一次；延迟**数字**仍取 WebSocket 的实时值，所以会比最右边那格新。
  缓冲区只在内存里，刷新即重来。实例详情页的延迟图表仍读历史 —— 那是用户主动打开、
  单节点一次的请求。
