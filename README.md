...
| 实例详情的负载 / 延迟历史；首页异常空洞的一次性回填 | `GET /api/history/all` |
...
- **首页延迟柱状图默认不查历史**：`/api/history/all` 会扫该节点整段时间窗口的历史行，
  首页给每台节点每分钟查一次会让后端 D1 读行翻几十倍（后端作者实测约 60 倍，
  30 秒上报约 120 倍）。正常节点仍只从这几个来源取数，不额外查库：
  ...
  2. **后端窗口（兜底）**：`/api/servers` 的 `servers[].ping` / `servers[].loss`，最近一小时
     30 个槽位、每 2 分钟一个。只用来顶上页面打开之前、或标签页被挂起的那段时间 —— 它不是把
     探测结果聚合出来的，而是拿最近一次结果**向后填充**，保真度不足（实测有节点整整 48 分钟
     都是同一个数字；丢包那边每格只装一次探测结果，值非 0 即 16%，会系统性漏采）。
  3. **详情页回灌**：用户点开实例详情页时本来就查了那台节点的历史，那份真实采样会顺手并进
     首页缓冲。不额外发请求，所以**点开过详情页的节点，首页那一小时更准**。

  4. **异常空洞按需回填**：首页确实展示 Ping、节点在线，且合并后的某条线路出现内部至少两格
     或末尾至少四格的明显断档、WSS 本地样本也没补齐时，才对该节点查一次一小时真实历史。
     四条线路共享这一次请求；正常完整窗口、只有末尾几分钟网格偏移、未展示 Ping 的节点不查。
     成功结果在浏览器侧记 10 分钟，并与现有历史接口的 in-flight/短期缓存合并去重，刷新或三网
     卡片重复挂载也不会短时间内重复请求。
  ...
  **卡片的丢包率和详情页图表仍可能对不上**：详情页读的是 30 秒一行的历史，而首页在浏览器
  实测覆盖不到的时段只能用上面那份窗口 —— 窗口每格只是一次探测结果，短促的丢包整段漏掉是常事
  （线上实测同一小时：窗口算 1.07%，历史算 5.52%）。**刚打开面板时如果检测到明显空洞会按节点
  自动补一次；没有明显空洞时，页面开着越久越接近，点进一次详情页也可以立刻拉齐那一小时。**
  要看准确值以详情页为准。
