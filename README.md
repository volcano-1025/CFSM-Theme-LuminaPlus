...
页面切到后台满 30 秒会断开 WebSocket、暂停轮询，切回来先补一次 `/api/servers` 再重连 —— 后端只要有前端
...
- **延迟探测**：CF-Server-Monitor 的探测点是后端固定的几条线路 —— 电信 / 联通 / 移动 / BD，
  ...
  （v1.2.14 起，设置页里 1~8 条随便选，不再固定三条）；关掉后是单线路模式，
  ...
  **在卡片上换线路**（多线路模式的大卡 / 小卡）：点线路名，从这台节点有数据的线路里挑一条换到
  这一行；挑的线路已经在别的行显示时两行互换。换的先存在当前浏览器、每台节点各记各的；菜单里的
  「恢复默认」把这台节点换回站点设置。**登录站长**换好后到设置页点「保存到后端」，就成了所有访客的
  默认线路（存进主题配置的 `homepagePingLineOverrides`，「复制配置 JSON」也带上），保存成功后本机那份
  自动清掉。站长以后调线路，没换过的行照样跟着走。换线路不发任何请求（首页缓冲区本来就带着全部线路的数据）。
...
- **主题设置存在本地，登录站长可发布到后端**：设置页右上角的「保存到本机」把配置存进当前设备的
  浏览器 localStorage，只影响这台设备。

  **登录站长**：还多一个「**保存到后端**」——把当前配置直接写到后端，所有设备与访客都会用这套。
  流程是先「保存到本机」在这台设备预览，确认后「保存到后端」发布；发布成功后本机自动跟随。
  它只更新站点的主题配置，不碰站点标题、背景图、CSP 等其它外观设置。「复制配置 JSON」对登录站长
  隐藏（有了「保存到后端」用不到了）。

  **未登录 / 纯静态或跨域部署**：没有「保存到后端」，改用「复制配置 JSON」——把导出的完整快照
  （含配色等设置页之外的项）粘到后台「外观设置 → 主题自定义配置」保存，效果一样（键名与主题设置一致）。

  **「改用后端配置」**：丢掉本机存过的设置（含配色、首页卡片上换过的线路），改用后端当前的配置。某台设备存过本机设置后，
  后端改的配置在那台设备上压不过来（本地优先），在那台设备点一次「改用后端配置」即可跟随。
  ...
  **后台保存后不是立刻生效，等一两分钟再看**：后端对外观设置有约 2 分钟的内存缓存，而且每个
  Worker 实例各存一份 —— 刚保存完换个浏览器打开，很可能还是旧的一份。要确认后端到底下发了什么，
  在站点页面的控制台跑
