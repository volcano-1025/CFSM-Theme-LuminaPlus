...
- **主题设置存在本地**：访客的主题设置保存在浏览器 localStorage，只影响当前设备。站长要给
  所有访客统一预设，有两条等价的路，都写的是同一份完整快照（含配色等设置页之外的项）：
  设置页的「同步后台配置」会丢掉本机那份、改用后台最新的预设。

  **多设备同步（登录站长）**：把设置调好后，点设置页右上角的「**保存到站点**」——
  以登录身份把配置直接写到后端，无需再手动粘 JSON。保存成功后本机会自动同步到这套配置。
  这个按钮只有登录站长看得到，只更新站点的主题配置，不碰站点标题、背景图、CSP 等其它外观设置。

  **多设备同步（手动，或未登录 / 纯静态部署）**：点「复制配置 JSON」，粘贴到后台「外观设置」的
  「主题自定义配置」里保存即可（键名与主题设置一致）。之后所有设备、所有访客都以它为默认值；
  某台设备如果已经存过本机设置，后台改的 JSON 压不过来（本地优先），在那台设备上点一次
  「同步后台配置」即可跟随。
