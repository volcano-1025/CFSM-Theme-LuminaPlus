...
- **延迟探测**：CF-Server-Monitor 的探测点是后端固定的几条线路 —— 电信 / 联通 / 移动 / BD，
  ...
  **在卡片上换线路**（多线路模式的大卡 / 小卡）：点线路名，从这台节点有数据的线路里挑一条换到
  这一行；挑的线路已经在别的行显示时两行互换。只存在当前浏览器、每台节点各记各的，不写后端、
  不改站点设置 —— 站长以后调线路，没换过的行照样跟着走；菜单里的「恢复默认」把这台节点换回站点设置。
  换线路不发任何请求（首页缓冲区本来就带着全部线路的数据）。
...
- **主题设置存在本地，登录站长可发布到后端**：设置页右上角的「保存到本机」把配置存进当前设备的
  ...
  **「改用后端配置」**：丢掉本机存过的设置（含配色），改用后端当前的配置。某台设备存过本机设置后，
