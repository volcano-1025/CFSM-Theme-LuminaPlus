...
- **首页延迟柱状图不自动查历史**：`/api/history/all` 会扫该节点整段时间窗口的历史行，
  ...
  5. **打开页面时的自检弹窗（问一句，仍然是手动）**：页面加载后自检一次上面第 2 条那份窗口的
     成色 —— 柱子空缺得厉害（面板里四分之一以上的节点、非掉线格空掉三成）就弹窗问要不要按第 4
     条刷一次，弹窗里写明这次要发几个请求、后端大约读多少行 D1。**自检本身一个请求都不发**，
     只看已经拿到的数据；发请求的仍然只有你点的那一下，「跳过」什么都不做。
     嫌它打扰可以在主题设置的「主页延迟检测」里关掉（「延迟数据不完整时弹窗提醒」），关了之后
     右上角的刷新按钮照常可用。刚刷过（30 分钟内）也不会问。
