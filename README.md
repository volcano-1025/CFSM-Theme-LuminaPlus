...
- **首页延迟柱状图不自动查历史**：`/api/history/all` 会扫该节点整段时间窗口的历史行，
  ...
  2. **后端窗口（兜底）**：`/api/servers` 的 `servers[].ping` / `servers[].loss`，最近一小时
     20 个槽位、每 3 分钟一个（卡片上的格子数与它一一对应）。只用来顶上页面打开之前、
     或标签页被挂起的那段时间。
