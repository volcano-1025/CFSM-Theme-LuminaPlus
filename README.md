...
- **首页延迟柱状图不查历史**：`/api/history/all` 会扫该节点整段时间窗口的历史行，
  ...
  **卡片的丢包率和详情页图表对不上是正常的**：两者数据源不同 —— 卡片用上面那个 2 分钟窗口，
  ...
  单次采样太跳）。丢包率是后端给的百分比（例如 6 个包丢 1 个 = 16%），不是「丢了几个包」。

  **柱子以浏览器实测为准**：一小时 30 格。后端 `/api/servers` 给的探测窗口是拿最近一次结果
  向后填充出来的（实测有节点 48 分钟都是同一个数字），保真度不足，所以只用来顶上页面打开之前、
  或标签页被挂起的那段时间；其余时段用浏览器自己从 WebSocket 攒到的实测值。两种来源疏密不同，
  每个样本按**它代表的时长**计权，丢包率因此不会随页面开着的时长漂移。延迟**数字**取 WebSocket
  的实时值，所以会比最右边那格新。缓冲区写进 localStorage 保留一小时，刷新后接着用。
  实例详情页的延迟图表仍读历史 —— 那是用户主动打开、单节点一次的请求；那份历史会顺手回灌
  首页缓冲，所以**点开过详情页的节点，首页那一小时的数据更准**（不额外发请求）。
