...
- **首页延迟柱状图不自动查历史**：`/api/history/all` 会扫该节点整段时间窗口的历史行，
  首页给每台节点每分钟查一次会让后端 D1 读行翻几十倍（后端作者实测约 60 倍，
  30 秒上报约 120 倍）。首页平时从这几个来源取数，都不额外查库：

  1. **浏览器实测（主来源）**：`/api/servers` 与 WebSocket 里一直都有的 `ping_*` / `loss_*`
     当前值。探针约 60 秒测一次，值变了就记一个样本，平稳时每 2 分钟留一个心跳。
     缓冲区写进 localStorage 保留一小时，刷新后接着用。
  2. **后端窗口（兜底，且会过滤）**：`/api/servers` 的 `servers[].ping` / `servers[].loss`，
     最近一小时 30 个槽位、每 2 分钟一个。只用来顶上页面打开之前、或标签页被挂起的那段时间 ——
     它不是把探测结果聚合出来的，而是**复制距离最近的那个样本**填满 30 格（后端最近邻没有距离
     上限），而且桶只在有人开着面板时才攒。于是「30 个点里有几个是真的」= 有人看着的分钟数 ÷ 2。

     主题因此会**丢掉连续 4 格以上「四条线路的延迟和丢包同时逐字节相同」的段落**，那些格子留空。
     所以**刚加进来的节点柱子大片空白是预期行为，不是 bug**（实测某新节点：窗口 30 格全是
     `1/1/1`，而历史表里只有 13 行、跨度 6.3 分钟）。副作用是延迟常年不变的节点（例如探测目标
     在本机）窗口也会被整段丢掉，只剩实测和刷新回灌的部分 —— 宁可留空也不画编出来的数据。
  3. **详情页回灌**：用户点开实例详情页时本来就查了那台节点的历史，那份真实采样会顺手并进
     首页缓冲。不额外发请求，所以**点开过详情页的节点，首页那一小时更准**。
  4. **右上角的刷新按钮（手动）**：点一下逐台拉一次一小时历史回灌，等同于「把每台节点的详情页
     都点开一遍」，同时在途的请求最多 4 个（总请求数仍是每台一个）。**只有点击会触发，没有任何
     自动轮询** —— 差别就是上面那 60 倍：一台节点一小时的行数 = `3600 ÷ 上报间隔`，点一次是各
     节点之和（8 台 30/60 秒上报约 900 行），而每分钟自动拉一次是它的 60 倍。后端对 1 小时档有
     60 秒服务端缓存，连点不会真的重复读库。刚打开面板想立刻看到准确的一小时，按它。

     刷完按钮会短暂变成绿色对勾并弹出「延迟数据已更新 · N 台」。**距上次刷新不到 30 分钟再点会
     先提醒一次**（琥珀色，这一次不发请求），趁提示还在的 15 秒内再点一下就照刷 —— 是提醒不是
     禁止，上次刷新时间存 localStorage，刷新页面绕不过。

  两种来源疏密不同，每个样本按**它代表的时长**计权（到下一次采样为止），丢包率因此不会随
  页面开着的时长漂移。一个样本最长延续 5 分钟；窗口里明确下发了 null 的槽位（探测失败）
  附近没有实测点时仍然留空，不会被相邻样本填平。

  **节点掉线后柱子转红**：以掉线节点最后一次上报为界，之后的样本一律不采信
  （后端窗口在节点掉线后可能照样按墙钟铺格子、沿用最后一个已知值），整格都在这之后的
  柱子涂红、悬浮显示「离线」。掉线约 2 分钟后最右边一格变红，之后每 2 分钟往左多一格，
  一小时后整条全红。

  **卡片的丢包率和详情页图表仍可能对不上**：详情页读的是 30 秒一行的历史，而首页在浏览器
  实测覆盖不到的时段只能用上面那份窗口 —— 窗口那 30 个点里，真正测到的只有「有人开着面板的
  分钟数 ÷ 2」个，其余是后端复制最近一个样本填出来的（线上实测同一小时：窗口算 1.07%，
  历史算 5.52%）。**刚打开面板时差距最大，页面开着越久越接近；按一下右上角的刷新按钮、
  或点进一次详情页，可以立刻拉齐那一小时。** 要看准确值以详情页为准。

  卡片上两个数字的口径**与原版 Komari 主题一致**：**延迟是最近一次采样的值**（瞬时量，
  看最新的才有意义，取自 WebSocket 实时值，所以会比最右边那格新），**丢包率是最近一小时的
  加权平均**（丢包本来就是一段时间内的比率，单次采样太跳）。丢包率是后端给的百分比
  （例如 6 个包丢 1 个 = 16%），不是「丢了几个包」。
