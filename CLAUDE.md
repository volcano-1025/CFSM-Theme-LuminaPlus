...
- **首页延迟条以后端窗口为准，本地累积只补缺口**（v1.2.7，`mergeWindowWithLocal`）：两个来源
  口径不同 —— 窗口是后端按 2 分钟网格算好的，本地是 WS 当前值 20~50 秒攒一个。早先按时间戳
  取并集，同一格里混两种样本，丢包的加权平均既不等于窗口自己算的、也不等于详情页历史算的，
  且本地点越攒越多、比例随页面开着的时长漂移（线上反馈的「首页数据不对」多半是这个）。现在
  只有窗口相邻两点间隔超过网格步长两倍（含窗口整体卡住、末点迟迟不动）才拿本地点填那一段，
  正常情况下一个都不掺。步长取相邻间隔的**中位数**：末尾那个「当前」点常常离上一格好几分钟，
  取平均/最大会被它带偏。改这里要同时想到 v1.2.5 修的「中间缺一大段」别复发。
  补进空洞的本地点必须**先按网格步长抽稀**（一格最多一个）：丢包率是按样本条数加权的，本地点
  20~50 秒一个、窗口 2 分钟一个，原样塞进去那段就有三四倍权重，卡片丢包率被它拖着走
  （实测真实 5.0% 显示成 6.6%）。补空洞是为了别让图缺一块，不是让这段说话更大声。
  窗口里「格子在、值是 null」的槽位（后端那轮探测没出结果）会被邻近本地样本顶替 —— 图表对
  明确没值的槽位是真的留空的，不顶替就平白空一格（并集时代顺手盖住了，切成窗口口径后立刻露出来）。
  另：卡片上那个「当前延迟」数字不走窗口，由 `withLiveLatency` 换成 WS 实时值（窗口末点会滞后
  一格 + 快照本身旧几十秒，只看数字会以为卡住了）；丢包率不能这么办 —— 它是整段窗口的加权
  平均，取最后一次采样会把「6 个包丢 1 个」顶成 16% 而柱子还是全绿。
