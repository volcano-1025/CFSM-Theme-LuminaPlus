...
- **首页卡片和详情页图表的丢包率对不上是正常的，别去「修」**：分辨率差着一个数量级 ——
  首页窗口一行 3 分钟（一小时 20 行），详情页历史约 30 秒一行。短促的丢包在窗口里要么整段
...
- **后端探测窗口 2026-08-23 换了实现，老的那套坑基本作废**（站长口径：改成从 D1 取一小时的
  ping/loss，返回条数由 30 条改为 20 条）。当天实测 8 台佐证：每台 20 行、步长 179~180 秒、
  跨度 56.7~56.9 分钟，**各行数值都在变，没有复印段**。
...
- **柱子格数只有一个来源，CSS 不许再写一遍**：四种视图统一读
  `HOMEPAGE_PING_BUCKET_COUNT`（v1.2.10 起 = 20，对齐后端一小时 20 行，一格正好 3 分钟）。
  2026-08-23 栽过：JS 从 18 改成 20，`compact-node-card.css` 里还写死着
  `grid-template-columns: repeat(18, ...)`，多出来两根直接掉到第二排。现在那里是
  `grid-auto-flow: column` + `grid-auto-columns`，列数由柱子数量自己撑开。
  开页自检的 `HEALTH_BUCKET_COUNT` 也要跟着走，否则「空缺比例」的阈值会飘（有单测钉住）。
- **后端可以关掉首页的详细 ping/loss**（`sysConfig.show_three_net_details`，后端 2026-08-23
  新增）：关掉时 `servers[].ping[]` / `loss[]` 那一小时窗口不再下发，只剩每台当前的单条
  `ping_ct/cu/cm/bd`。主题据此回退两处 —— 三网三条线改走单线路（否则三条空线）、开页自检
  不跑（窗口本来就不下发，柱子空是预期，再弹窗就是每次开页误报）。
  **默认必须是 true**：老后端不下发这个字段而它们一直输出详细数据，默认 false 会让存量站点
  的三网线全部消失。
- **缓冲区保留期只比后端窗口多零点几分钟，是擦边状态**：`SAMPLE_TTL_MS` 正好 60 分钟，而后端
  20 行跨 57.3 分钟、最新一行本身能旧到 3 分钟 —— 最老那行到主题手里常常已经 60.2 分钟大。
  2026-08-23 实测 8 台：7 台的最老行一进来就被当成过期丢掉（只剩 19 条），只有 `nat`
  （59.99 分）保住。目前**看不出来**，因为 `buildPingBuckets` 里最老样本的向前回填
  （`leadingHoldMs`，按它到下一个样本的实际间隔补，不是全局中位数）正好盖住了空出来的那格。
  等于靠外推顶着一个本来有真数据的格子，余量不到一分钟。后端节奏一变就会露出来 ——
  再收到「第一格空」先量这个：`最老行年龄 = 窗口跨度 + 最新一行年龄`，超过 60 分钟就是它。
  放宽 TTL 的副作用是卡片丢包率的平均窗口跟着变宽（那是整段缓冲区的加权平均），
  会动「丢包率与柱子同源」这条口径，所以没动。
...
`Powered by CF-Server-Monitor · Theme by LuminaPlus` 两个 GitHub 链接。461 项测试通过。
...
3. `SAMPLE_TTL_MS` 与后端窗口跨度只差零点几分钟（见「容易踩的坑」），已定位未改。
