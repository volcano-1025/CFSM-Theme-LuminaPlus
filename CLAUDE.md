...
- **后端探测窗口会间歇性停止滑动**：实测 7 台节点一致，2 分钟网格铺到约 35 分钟前就停、
  末尾直接追加一个「当前」点，跨度反而有 90 分钟（过一阵自己追上）。`seedPingHistory` 因此
  按时间戳与本地缓冲取并集；且超上限时必须**按时间抽稀**而不是 `slice(-N)` —— WSS 下本地
  约 20 秒攒一个、很快顶满 96 条却只覆盖半小时，砍最老的会把窗口里较早的点整段丢掉。
...
v1.2.5 已发布，`dist` 头是产物提交 `afe2d9f`（2026-08-17）。主题商店里的版本登记由站长
