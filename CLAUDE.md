...
- **首页不许「自动」发起 `/api/history/all` 请求**（v1.2.8 改口径，原来是一概不许）：
  ...
  ③ **用户点右上角刷新按钮**（`refreshPingHistory`，每台一次 `hours=1`，并发上限 4）。
...
v1.2.8 开发中：首页右上角加了手动刷新按钮（见上面改口径的那条硬约束），并新增
`dropBackfilledRuns` 丢弃后端窗口里的复印段。已跑完 typecheck / lint / test（400 项）。
刷新按钮那部分已推 preview（产物 `838b54c`）并线上验证过；**复印段过滤还没进 preview**。
