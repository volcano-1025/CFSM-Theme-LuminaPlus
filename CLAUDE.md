...
- **采样口径是「一次探测记一个样本」**（`recordPingSample`）：探针 60 秒才测一次，WS 每 2 秒把
  同一个结果重推一遍，所以「值变了」等价于「新探测落地了」，一到就记；值没变每 2 分钟留一个心跳
  （只为柱子有点可画，不影响加权）。早先「值没变 50 秒、值变了 20 秒」的不对称阈值会在每次丢包的
  **切换处**多记一个样本，丢包样本被系统性多记。
- **每个样本按它代表的时长计权，不能一条算一条**（`assignWeights`）：丢包率是按样本条数加权
  平均的，而窗口 2 分钟一个点、本地样本疏密不定 —— 按条数算，密的那段说话就大好几倍，
  比例还随页面开着的时长漂移（早先「首页数据不对」的根）。**时长要按「到下一次采样为止」算，
  不是「到前后邻居的中点」** —— 后者在疏密不均处会翻车：平稳时 2 分钟一个心跳、丢包那次立刻记下，
  一个只代表 60 秒的丢包样本会拿到 90 秒权重，丢包率凭空高一半（构造实测高估 71%）。口径必须
  和 `buildPingBuckets` 铺格子的方式一致。基数 `WEIGHT_SCALE = 8`（取整误差不能太大）。
